...
class ApplicantDetails(webapp.RequestHandler):
	"""
	...
	def __init__(self):
		...
		self.results = []
		self.applicant = system.current_user
		self.majors = [major for major in data_models.Majors.all()]
		self.phd = 'True' if self.applicant and self.applicant.phd else 'False'
		self.supervising_professors = [supervising_professor for supervising_professor in data_models.Instructors.all()]
		self.citizen = 'True' if self.applicant and self.applicant.citizen else 'False'
		self.native_english_speaker = 'True' if self.applicant and self.applicant.native_english_speaker else 'False'
		self.programming_languages = [programming_language for programming_language in data_models.ProgrammingLanguages.all()]
		self.applicant_programming_languages = [applicant_programming_language for applicant_programming_language in data_models.ApplicantsProgrammingLanguages.gql('WHERE applicant = :1', self.applicant)]
		...
		self.applicant_courses = [applicant_course for applicant_course in data_models.ApplicantsCourses.gql('WHERE applicant = :1', self.applicant)]
	...
	def template(self):
		...
		template_values = {
			...
			'applicant_programming_languages': self.applicant_programming_languages,
			'specializations': self.specializations,
			'courses': self.courses,
			'applicant_courses': self.applicant_courses
		}
		path = os.path.join(os.path.dirname(__file__), 'templates', 'applicantDetails.html' if self.logged_in else 'index.html')
